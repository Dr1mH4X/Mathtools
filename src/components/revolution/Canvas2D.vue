...
function drawGrid(
  ...
  for (let x = xStart; x <= xRange[1]; x += xStep) {
    ...
    for (let sx = x + subStep; sx < x + xStep && sx <= xRange[1]; sx += subStep) {
  ...
  for (let y = yStart; y <= yRange[1]; y += yStep) {
    ...
    for (let sy = y + subStep; sy < y + yStep && sy <= yRange[1]; sy += subStep) {
...
function drawLabels(
  ...
  for (let x = xStart; x <= xRange[1]; x += xStep) {
    ...
    const labelY = Math.min(Math.max(yZero + 6, 4), canvasHeight.value - 16);
...
function drawRegion(
  ...
  if (region.upperProfile.length < 2 || region.lowerProfile.length < 2) return;

  ctx.save();

  // Fill the region between upper and lower profiles
  const fillColor = isDark ? "rgba(79, 110, 247, 0.15)" : "rgba(79, 110, 247, 0.12)";
  ...
  ctx.strokeStyle = isDark ? "rgba(79, 110, 247, 0.3)" : "rgba(79, 110, 247, 0.25)";
  ctx.lineWidth = 1;
  ctx.stroke();
...
function drawHatching(
  ...
  // Draw hatching lines
  ctx.strokeStyle = isDark ? "rgba(79, 110, 247, 0.12)" : "rgba(79, 110, 247, 0.1)";
  ctx.lineWidth = 1;
  const spacing = 8;
...
function drawCurves(
  ...
  for (let ci = 0; ci < props.curves.length; ci++) {
    ...
    const color = curve.color ?? defaultCurveColors[ci % defaultCurveColors.length]!;
    ...
    try {
      ...
      if (curve.type === "x_const") {
        ...
        ctx.textAlign = "left";
        ctx.textBaseline = "top";
        ctx.fillText(curve!.equation || `x = ${curve!.expression}`, cx + 6, 12);
        ...
        ctx.fillText(curve!.equation || `y = ${curve!.expression}`, 12, cy - 6);
      } else {
        ...
        for (const p of pts) {
          ...
          if (prevPt && Math.abs(cp.cy - prevPt.cy) > canvasHeight.value * 0.8) {
            ctx.stroke();
            ctx.beginPath();
            started = false;
          }
        ...
        const labelIdx = Math.min(Math.floor(pts.length * 0.75), pts.length - 1);
        const labelPt = pts[labelIdx];
        if (labelPt && isFinite(labelPt.y)) {
          const lp = mathToCanvas(labelPt.x, labelPt.y, padding, xRange, yRange);
          if (
            lp.cx > 20 &&
            ...
            const label = curve!.equation
              || ((curve!.type === "y_of_x" ? "y = " : curve!.type === "x_of_y" ? "x = " : "") + curve!.expression);
...
function drawRotationIcon(
  ...
  if (props.axis === "x") {
    ...
    const converted = mathToCanvas(mid, props.axisValue, padding, xRange, yRange);
    cx = converted.cx;
    cy = converted.cy;
  } else {
    const midY = 0;
    const converted = mathToCanvas(props.axisValue, midY, padding, xRange, yRange);
    cx = converted.cx;
    cy = converted.cy;
  }
...
watch(
  () => [props.curves, props.region, props.xMin, props.xMax, props.axis, props.axisValue],
  () => {
...
<template>
  <div ref="containerRef" class="canvas2d-container">
    <canvas
      ref="canvasRef"
      class="canvas2d"
    ...
    <div class="canvas2d-controls">
      <button class="btn btn-sm btn-icon" title="Reset view" @click="resetView">
      ...
      <button class="btn btn-sm btn-icon" title="Zoom in" @click="viewScale *= 1.2; draw()">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
             stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <button class="btn btn-sm btn-icon" title="Zoom out" @click="viewScale *= 0.8; draw()">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
             stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <circle cx="11" cy="11" r="8" />
          <line x1="21" y1="21" x2="16.65" y2="16.65" />
...
.canvas2d-controls .btn-icon:hover {
  opacity: 1;
}
